...
function zerif_setup() {
	...
	add_image_size( 'zerif-post-thumbnail', 225, 305, true );
...
function zerif_scripts() {
	...
	wp_enqueue_style( 'zerif_cv_style', get_template_directory_uri() . '/css/cv.css', array( 'zerif_style' ), ZERIF_LITE_VERSION );
	wp_style_add_data( 'zerif_ie_style', 'conditional', 'lt IE 9' );
   /* Slick slider*/
    wp_enqueue_style( 'zerif_slcik_style', get_template_directory_uri() . '/css/slick.css' );
    wp_enqueue_style( 'zerif_slcik_theme_style', get_template_directory_uri() . '/css/slick-theme.css' );
	wp_enqueue_style( 'zerif_slider_style', get_template_directory_uri() . '/css/theme_slider.css' );
    if ( wp_is_mobile() ) {
    ...
    /* Slick slider*/
    wp_enqueue_script( 'zerif_slick', get_template_directory_uri() . '/js/slick.min.js', array( 'jquery' ), ZERIF_LITE_VERSION, true );
...
function zerif_wp_page_menu() {
	...
	echo '<ul class="nav navbar-nav navbar-center responsive-nav main-nav-list">';
...
if ( ! class_exists( 'zerif_ourfocus' ) && zerif_check_if_old_version_of_theme() ) {
	...
	class zerif_ourfocus extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			echo '<div class="col-lg-2 col-sm-2 focus-box" data-scrollreveal="enter left after 0.15s over 1s">';
...
/***
 * single page for curent category
 */
add_filter('single_template', create_function(
	...
	register_sidebar( array(
		...
		'after_widget'  => '</div>',
		'before_title'  => '<div class="section-header"><h2 class="dark-text">',
		'after_title'   => '</h2></div>',
	) );
register_sidebar( array(
		'name'          => 'Карта в главном странице',
		'id'            => 'custom-maps-widget',
                
		'before_widget' => '<div class="cmw-widget">',
		'after_widget'  => '</div>',
		'before_title'  => '<h2 class="cmw-title white-text zerif_hidden_if_not_customizer ">',
		'after_title'   => '</h2>',
	) );

register_sidebar( array(
		'name'          => 'Галлерея в главном странице',
		'id'            => 'custom-main-galerey-widget',

		'before_widget' => '<div class="col-lg-12 col-md-12 column zerif_about_us_center  ">',
		'after_widget'  => '</div>',
		'before_title'  => '<h2 class="cmw-title white-text ">',
		'after_title'   => '</h2>',
	) );
...
/***
 * get Month and day in kazakh language
 * @param string $day
 * @param string $month
 *
 * @return  translate array
 */
function translateDate( $day='Monday', $month='January' ) {
	$day_ar = array(
		'Monday'    => 'Дүйсенбі',
		'Tuesday'   => 'Сейсенбі',
		'Wednesday' => 'Сәрсенбі',
	...
	return array($day_ar[$day], $month_ar[$month]);
}
function get_data($var, $def=null){
