...
<!--section class="latest-news ' . ( ( is_customize_preview() && ( ! isset( $zerif_latestnews_show ) || $zerif_latestnews_show == 1 ) ) ? ' zerif_hidden_if_not_customizer ' : '' ) . '" id="latestnews">
    zerif_top_latest_news_trigger();
</section-->


<?php if (is_active_sidebar('custom-header-widget')) : ?>
    <section class="container" id="afisha" style="padding-bottom: 15px;">
    <?php dynamic_sidebar('custom-header-widget'); ?>
        <a href="/afisha" class="btn btn-danger custom-button pull-right btn-home" style="position:
       relative; left:  10px;">Толығырақ</a>
...
//	$zerif_latestnews_show = get_theme_mod( 'zerif_latestnews_show' );
    ...
    /*Галерея на главном */
if (is_active_sidebar('custom-main-galerey-widget')) : 
    echo '<section class="container">';
    dynamic_sidebar('custom-main-galerey-widget');
    echo '</section>';
     endif;
     /************/
    /* CONTACT US */
  ...
  if (current_user_can('edit_theme_options')) {
    ...
    ?>
     ...
     <?php if (is_active_sidebar('custom-maps-widget')) : ?>
            <div class="section-header">
            <?php dynamic_sidebar('custom-maps-widget'); ?>
            </div>
        <?php endif; ?>
        ...
        <?php
        echo ' </section> <!-- / END CONTACT US SECTION-->';
    endif;
    ?>
